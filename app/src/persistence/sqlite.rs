    ...
    conn.transaction::<(), Error, _>(|conn| {
        ...
        for (idx, window) in app_state.windows.iter().enumerate() {
            ...
            let new_window = NewWindow {
                ...
                vertical_sidebar_mode: Some(matches!(
                    window.vertical_sidebar_mode,
                    crate::app_state::VerticalSidebarMode::Tasks
                )),
            ...
            .map(
                |(((idx, window), tabs_for_window), tab_groups_for_window)| {
                    ...
                    WindowSnapshot {
                        ...
                        vertical_sidebar_mode: match window.vertical_sidebar_mode {
                            Some(true) => crate::app_state::VerticalSidebarMode::Tasks,
                            // Existing rows predate the column and must retain
                            // the original Sessions sidebar behavior.
                            Some(false) | None => crate::app_state::VerticalSidebarMode::Sessions,
                        },
